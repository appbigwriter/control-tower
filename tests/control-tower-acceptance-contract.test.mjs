...
test('existing Control Tower routes match the acceptance contract', async () => {
  ...
  const provisionUi = await text('src/components/control-tower/ProjectProvisionForm.tsx')
  const provisionLib = await text('src/lib/control-tower/projects-create.ts')
  ...
  assert.match(provisionUi, /domain,/) 
  assert.match(provisionUi, /value=\{domain\}[\s\S]*required/)
  assert.match(provisionLib, /typeof raw\.domain !== 'string' \|\| raw\.domain\.trim\(\)\.length === 0/)
